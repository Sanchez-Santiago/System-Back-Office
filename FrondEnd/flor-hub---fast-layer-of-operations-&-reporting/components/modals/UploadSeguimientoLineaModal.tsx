...
interface UploadSeguimientoLineaModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const UploadSeguimientoLineaModal: React.FC<UploadSeguimientoLineaModalProps> = ({
  isOpen,
  onClose,
  onUpload,
  isUploading = false,
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-[700px] bg-white dark:bg-slate-900 rounded-[3vh] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300">
        {/* Header */}
        <div className="bg-gradient-to-br from-cyan-600 via-cyan-700 to-blue-700 p-[3vh] text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-[2vh]">
              ...
              <div>
                <h3 className="font-black uppercase tracking-widest text-[clamp(1rem,2vh,2.5rem)]">Actualizar Seguimiento Línea</h3>
                <p className="font-bold text-cyan-200 uppercase text-[clamp(0.7rem,1.2vh,1.5rem)]">Carga masiva por archivo</p>
              </div>
            </div>
            <button 
              onClick={handleClose}
        ...
        <div className="p-[3vh] space-y-[2.5vh]">
          {/* Info de columnas */}
          <div className="bg-cyan-50 dark:bg-cyan-900/20 rounded-[2vh] p-[2.5vh] border border-cyan-100 dark:border-cyan-800">
            <p className="font-black text-cyan-800 dark:text-cyan-300 uppercase tracking-widest mb-[1.5vh] text-[clamp(0.7rem,1.2vh,1.5rem)]">
              Columnas requeridas:
            </p>
            <div className="grid grid-cols-4 gap-[1.5vh]">
              {['DOCUMENTO', 'FECHA_NACIMIENTO', 'PROMO', 'PLAN_ID', 'PEDIDO_STL', 'NUMERO_CONTACTO', 'PEDIDO_SAP', 'ULTIMO_STATUS'].map((col) => (
                <div key={col} className="bg-white dark:bg-slate-800 rounded-[1.5vh] p-[1.5vh] text-center border border-cyan-200 dark:border-cyan-700">
