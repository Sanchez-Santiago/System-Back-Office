...
export class EstadoVentaController {
     ...
     */
    async getLastByVentaId(req, res) {
        try {
            const venta_id = Number(req.params.venta_id);
            if (!venta_id || venta_id < 1) {
                res.status(400).json({
     ...
     */
    async getAllLastEstado(req, res) {
        try {
     ...
     */
    async getByMultipleFilters(req, res) {
        try {
            const url = new URL(req.url);
            const filters = {
            ...
            if (venta_id)
                filters.venta_id = Number(venta_id);
            ...
            const estados = await this.estadoVentaService.getByMultipleFilters(filters);
            res.status(200).json({
                success: true,
                data: estados,
                count: estados.length,
...
//# sourceMappingURL=EstadoVentaController.js.map