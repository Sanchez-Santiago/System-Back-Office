...
export class AuthController {
    modeUser;
    authService;
    constructor(modeUser, authService) {
    ...
    async changePassword(params) {
        try {
            ...
            logger.info(`Usuario autenticado: ${authenticatedUser.email} (${authenticatedUser.rol})`);
            ...
            if (isSelfChange) {
                ...
                if (!result.success) {
                    throw new Error(`Validación fallida: ${result.error.errors.map((error) => error.message).join(", ")}`);
                }
