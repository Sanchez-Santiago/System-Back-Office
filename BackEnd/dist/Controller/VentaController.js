...
export class VentaController {
    ...
    correoController;
    correoService;
    portabilidadController;
    lineaNuevaController;
    /**
     ...
     */
    constructor(ventaModel, clienteModel, correoModel, lineaNuevaModel, portabilidadModel, planModel, promocionModel, estadoVentaModel) {
        ...
        this.lineaNuevaController = new LineaNuevaController(lineaNuevaModel, ventaModel, portabilidadModel);
        this.portabilidadController = new PortabilidadController(portabilidadModel, ventaModel, lineaNuevaModel);
    }
    async getAll(input) {
        try {
            if (input.pais) {
    ...
    async getByDateRange(input) {
        try {
            const ventas = await this.ventaService.getByDateRange(input.start, input.end);
            return ventas;
        }
        catch (error) {
    ...
    async getVentasWithPagination(query) {
        try {
    ...
    async getVentaByDateRange(query) {
        try {
            const validation = this.ventaService.validateDates(query.start.toISOString(), query.end.toISOString());
            if (!validation.isValid) {
                return { success: false, message: validation.errors?.join(", ") };
            }
            const ventas = await this.ventaService.getByDateRange(query.start, query.end);
    ...
    async getVentaByParam(param, type) {
        try {
            let venta;
            switch (type) {
                case "sds":
    ...
    async createFullVenta(request, userId) {
        ...
        try {
            ...
            logger.debug(`Verificando existencia de cliente: ${ventaData.cliente_id}`);
            ...
            if (ventaValidada.plan_id) {
                logger.debug(`Validando plan ${ventaValidada.plan_id} pertenece a empresa ${idEmpresa}`);
                const planValidation = await this.ventaService.validatePlan(ventaValidada.plan_id, idEmpresa, this.planService);
            ...
            if (ventaValidada.promocion_id) {
                logger.debug(`Validando promoción ${ventaValidada.promocion_id} pertenece a empresa ${idEmpresa}`);
                const promoValidation = await this.ventaService.validatePromocion(ventaValidada.promocion_id, idEmpresa, this.promocionService);
            ...
            if (ventaValidada.chip === "SIM" && correoValidado) {
                try {
                    ...
                    const nuevoCorreo = await this.correoController.create(correoValidado);
                    ...
                    logger.info(`✅ Correo creado exitosamente: SAP ${nuevoCorreo.sap_id}`);
                }
                catch (error) {
                    logger.error("Error al crear correo:", error);
                    return {
                        success: false,
                        message: `Error al crear correo: ${error.message}`,
                    };
                }
            }
            ...
            try {
                ...
                try {
                    ...
                    logger.error("Error al eliminar venta durante rollback:", deleteError);
                }
                // Eliminar correo si fue creado
                if (correoCreado) {
                    try {
                        await this.rollbackCorreo(correoCreado.sap_id, `Error en post-procesamiento: ${error.message}`);
                        logger.info("✅ Rollback de correo completado");
                    }
                    catch (rollbackError) {
                ...
                return {
                    success: false,
                    message: `Error en post-procesamiento: ${error.message}`,
                };
            }
     ...
     */
    async rollbackCorreo(sapId, reason) {
        try {
     ...
     */
    async postProcessVenta(venta, portabilidad) {
        logger.debug(`Post-procesando venta ${venta.venta_id} de tipo ${venta.tipo_venta}`);
    ...
    async getVentasUI(req, res) {
        try {
            ...
            res.status(500).json({
                success: false,
                message: isDev ? error.message : "Error al obtener ventas",
            });
        }
    }
    /**
     ...
     * Router → Controller → Service → Model
     */
    async getVentaDetalleCompleto(req, res) {
        try {
            const { id } = req.params;
