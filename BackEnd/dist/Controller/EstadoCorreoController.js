...
export class EstadoCorreoController {
    ...
    async getBySAP({ sap }) {
        try {
            ...
            const estados = await this.service.getBySAP({ sap });
    ...
    async create(input) {
        try {
            ...
            if (!validationResult.success) {
                throw new Error(`Validación fallida: ${validationResult.error.errors.map((error) => error.message).join(", ")}`);
            }
            const estado = await this.service.create(validationResult.data);
            logger.info(`Estado creado exitosamente: ${estado.estado_correo_id}, SAP: ${estado.sap_id}, Estado: ${estado.estado}`);
            return estado;
        }
        catch (error) {
    ...
    async update(params) {
        try {
            ...
            // Validar con Zod
            const validatedInput = EstadoCorreoUpdateSchema.partial().parse(params.input);
    ...
    async bulkCreate(ctx) {
        try {
            ...
            for (const estado of estadosConUsuario) {
                const result = EstadoCorreoCreateSchema.safeParse(estado);
                if (result.success) {
            ...
            const result = await this.service.bulkCreate(estadosValidos);
            ctx.response.status = 201;
            ctx.response.body = {
                success: true,
                message: `Se actualizaron ${result.length} estados de correo`,
                data: result,
                count: result.length,
            };
        }
            ...
            manejoDeError("Error en bulkCreate EstadoCorreo", error);
        }
    }
}
//# sourceMappingURL=EstadoCorreoController.js.map