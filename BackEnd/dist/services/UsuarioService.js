...
export class UsuarioService {
    ...
    async getAll(params) {
        try {
            ...
            logger.info(`Obteniendo usuarios - Página: ${page}, Límite: ${limit}`);
            const users = await this.modeUser.getAll(params);
            if (!users || users.length === 0) {
                return undefined;
            }
            // ✅ Ya no necesitamos filtrar password_hash porque no está en la consulta
            const usersWithSecurity = users.map((user) => UsuarioSecuritySchema.parse(user));
    ...
    async getById({ id }) {
        try {
            ...
            throw new Error(`Error al obtener usuario por ID: ${error instanceof Error ? error.message : "Error desconocido"}`);
        }
    }
    /**
    ...
    async getByEmail({ email }) {
        try {
            ...
            throw new Error(`Error al obtener usuario por email: ${error instanceof Error ? error.message : "Error desconocido"}`);
        }
    }
    /**
    ...
    async getByLegajo({ legajo }) {
        try {
            ...
            throw new Error(`Error al obtener usuario por legajo: ${error instanceof Error ? error.message : "Error desconocido"}`);
        }
    }
    /**
    ...
    async getByExa({ exa }) {
        try {
            ...
            throw new Error(`Error al obtener usuario por EXA: ${error instanceof Error ? error.message : "Error desconocido"}`);
        }
    }
    /**
    ...
    async exists(params) {
        try {
            ...
            throw new Error(`Error al verificar existencia de usuario: ${error instanceof Error ? error.message : "Error desconocido"}`);
        }
    }
    /**
    ...
    async changeStatus(params) {
        try {
            ...
            logger.info(`Cambiando estado de usuario ${params.id} a ${params.estado}`);
