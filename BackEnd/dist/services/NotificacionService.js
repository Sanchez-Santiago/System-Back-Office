...
export class NotificacionService {
    ...
    async crearNotificacion(params) {
        ...
        try {
            const result = await client.queryObject(`INSERT INTO mensaje (tipo, titulo, comentario, usuario_creador_id, referencia_id, fecha_creacion)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING mensaje_id`, [tipo, titulo, comentario, usuarioCreadorId, referenciaId || null, new Date()]);
            ...
            if (destinatariosIds && destinatariosIds.length > 0) {
                for (const usuarioId of destinatariosIds) {
                    await client.queryObject(`INSERT INTO mensaje_destinatario (mensaje_id, usuario_id, leida)
             VALUES ($1, $2, false)`, [mensajeId, usuarioId]);
                }
            }
    ...
    async crearNotificacionGlobal(params) {
        ...
        try {
            ...
            const usuariosConPais = await client.queryObject(`SELECT DISTINCT u.persona_id
         FROM usuario u
         INNER JOIN celula c ON u.celula = c.id_celula
         WHERE c.pais_venta ILIKE $1
         AND u.estado = 'ACTIVO'`, [pais]);
            // Obtener usuarios ACTIVOS cuyas células NO tienen país (reciben TODAS las notificaciones)
            const usuariosSinPais = await client.queryObject(`SELECT DISTINCT u.persona_id
         FROM usuario u
         INNER JOIN celula c ON u.celula = c.id_celula
         ...
         AND u.estado = 'ACTIVO'`);
            ...
            [...usuariosConPais.rows, ...usuariosSinPais.rows].forEach((row) => {
            ...
            // Crear el mensaje
            const mensajeResult = await client.queryObject(`INSERT INTO mensaje (tipo, titulo, comentario, usuario_creador_id, referencia_id, fecha_creacion)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING mensaje_id`, [tipo, titulo, comentario, usuarioCreadorId, referenciaId || null, new Date()]);
    ...
    async crearNotificacionPorPais(params) {
        ...
        const client = this.pgClient.getClient();
        try {
            const usuariosResult = await client.queryObject(`SELECT DISTINCT u.persona_id
         FROM usuario u
         INNER JOIN celula c ON u.celula = c.id_celula
         WHERE c.pais_venta ILIKE $1
         AND u.estado = 'ACTIVO'`, [pais]);
            const destinatariosIds = usuariosResult.rows.map((row) => row.persona_id);
            if (destinatariosIds.length === 0) {
     ...
     */
    async getPaisByEmpresaOrigenId(empresaOrigenId) {
        const client = this.pgClient.getClient();
        try {
            const result = await client.queryObject(`SELECT pais FROM empresa_origen WHERE empresa_origen_id = $1`, [empresaOrigenId]);
    ...
    getComentarioPlan(accion, nombre, pais) {
        switch (accion) {
    ...
    getTituloPromocion(accion, nombre) {
        switch (accion) {
    ...
    getComentarioPromocion(accion, nombre, pais) {
        switch (accion) {
    ...
    getTituloEmpresa(accion, nombre) {
        switch (accion) {
    ...
    getComentarioEmpresa(accion, nombre, pais) {
        switch (accion) {
