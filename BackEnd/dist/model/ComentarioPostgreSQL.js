...
export class ComentarioPostgreSQL {
    connection;
    ...
    mapRowToComentarioConUsuario(row) {
        const converted = convertBigIntToNumber(row);
        return {
            comentario_id: converted.comentario_id,
            titulo: converted.titulo,
    ...
    async add({ input }) {
        ...
        try {
            ...
            const result = await client.queryObject(`INSERT INTO comentario (
    ...
    async getById({ comentario_id, }) {
        const client = this.connection.getClient();
        try {
            const result = await client.queryObject(`SELECT * FROM comentario WHERE comentario_id = $1`, [comentario_id]);
    ...
    async update({ comentario_id, input, }) {
        const client = this.connection.getClient();
        try {
            const fields = [];
            const values = [];
            ...
            const result = await client.queryObject(`UPDATE comentario SET ${fields.join(", ")}
    ...
    async delete({ comentario_id }) {
        const client = this.connection.getClient();
        try {
            const result = await client.queryObject(`DELETE FROM comentario WHERE comentario_id = $1 RETURNING comentario_id`, [comentario_id]);
    ...
    // MÉTODOS ESPECÍFICOS
    // ======================
    async getAll({ page = 1, limit = 20, venta_id, usuario_id, tipo_comentario, fecha_desde, fecha_hasta, }) {
        const client = this.connection.getClient();
        ...
        try {
            ...
            const result = await client.queryObject(`SELECT * FROM comentario ${whereClause}
    ...
    async getByVentaId({ venta_id, page = 1, limit = 20, }) {
        const client = this.connection.getClient();
        const offset = (page - 1) * limit;
        try {
         ...
         LIMIT $2 OFFSET $3`, [venta_id, limit, offset]);
    ...
    async getByUsuarioId({ usuario_id, page = 1, limit = 20, }) {
        const client = this.connection.getClient();
        const offset = (page - 1) * limit;
        try {
            const result = await client.queryObject(`SELECT * FROM comentario
         WHERE usuarios_id = $1
         ORDER BY fecha_creacion DESC
    ...
    async esVentaDelVendedor({ venta_id, vendedor_id, }) {
        const client = this.connection.getClient();
        try {
            const result = await client.queryObject(`SELECT COUNT(*) as count FROM venta
         WHERE venta_id = $1 AND vendedor_id = $2`, [venta_id, vendedor_id]);
    ...
    async getCreadorId({ comentario_id, }) {
        const client = this.connection.getClient();
        try {
            const result = await client.queryObject(`SELECT usuarios_id FROM comentario WHERE comentario_id = $1`, [comentario_id]);
            return result.rows.length > 0
