...
export class EmpresaOrigenPostgreSQL {
    connection;
    ...
    async getByNombre({ nombre }) {
        ...
        try {
          ...
          pais
         FROM empresa_origen
         WHERE nombre_empresa ILIKE $1`, [nombre]);
            return result.rows.length > 0 ? result.rows[0] : undefined;
        }
        catch (error) {
    ...
    async getByPais({ pais }) {
        ...
        try {
         ...
         FROM empresa_origen
         WHERE pais ILIKE $1
         ORDER BY nombre_empresa ASC`, [`%${pais}%`]);
            return result.rows || [];
        }
        catch (error) {
    ...
    async add({ input }) {
        ...
        try {
            ...
            if (existing) {
                throw new Error(`Ya existe una empresa origen con el nombre: ${input.nombre_empresa}`);
            }
            const result = await client.queryObject(`INSERT INTO empresa_origen (nombre_empresa, pais)
         VALUES ($1, $2)
         RETURNING empresa_origen_id, nombre_empresa, pais`, [input.nombre_empresa, input.pais]);
        ...
        try {
            ...
            if (input.nombre_empresa !== undefined) {
                ...
                if (duplicate && duplicate.empresa_origen_id !== Number(id)) {
                    throw new Error(`Ya existe otra empresa origen con el nombre: ${input.nombre_empresa}`);
                }
            ...
            values.push(id);
            const result = await client.queryObject(`UPDATE empresa_origen
         SET ${fields.join(", ")}
         WHERE empresa_origen_id = $${paramIndex}
         RETURNING empresa_origen_id, nombre_empresa, pais`, values);
            if (!result.rows || result.rows.length === 0) {
    ...
    async delete({ id }) {
        ...
        try {
            ...
            const plansResult = await client.queryObject(`SELECT COUNT(*) as count FROM plan WHERE empresa_origen_id = $1`, [id]);
            const plansCount = plansResult.rows[0]?.count || 0;
            if (plansCount > 0) {
                throw new Error(`No se puede eliminar la empresa origen porque tiene ${plansCount} plan(es) asociado(s)`);
            }
            // Verificar si tiene promociones asociadas
            const promosResult = await client.queryObject(`SELECT COUNT(*) as count FROM promocion WHERE empresa_origen_id = $1`, [id]);
            const promosCount = promosResult.rows[0]?.count || 0;
            if (promosCount > 0) {
                throw new Error(`No se puede eliminar la empresa origen porque tiene ${promosCount} promoción(es) asociada(s)`);
            }
            // Eliminar la empresa origen
            const result = await client.queryObject(`DELETE FROM empresa_origen
         WHERE empresa_origen_id = $1
         RETURNING empresa_origen_id`, [id]);
            if (!result.rows || result.rows.length === 0) {
    ...
    async count() {
        try {
            const client = this.connection.getClient();
            const result = await client.queryObject(`SELECT COUNT(*) as count FROM empresa_origen`);
    ...
    async getWithStats({ id }) {
        ...
        try {
         ...
         GROUP BY eo.empresa_origen_id, eo.nombre_empresa, eo.pais`, [id]);
            return result.rows.length > 0 ? result.rows[0] : undefined;
        }
        catch (error) {
    ...
    // GET ALL WITH STATS
    // ======================
    async getAllWithStats({ page = 1, limit = 10, } = {}) {
        if (page < 1 || limit < 1) {
            throw new Error("page y limit deben ser mayores a 0");
        }
        const offset = (page - 1) * limit;
...
//# sourceMappingURL=empresaOrigenPostgreSQL.js.map