...
export class VentaPostgreSQL {
    ...
    async getAll(params = {}) {
        ...
        const result = await client.queryObject(`SELECT * FROM venta LIMIT $1 OFFSET $2`, [limit, offset]);
        logger.debug("Venta rows:", result.rows || []);
        return (result.rows || []).map((row) => convertBigIntToNumber(this.mapRowToVenta(row)));
    }
    ...
    async getById({ id }) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT * FROM venta WHERE venta_id = $1`, [id]);
        if (!result.rows.length)
            return undefined;
        return convertBigIntToNumber(this.mapRowToVenta(result.rows[0]));
    }
    async getBySDS({ sds }) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT * FROM venta WHERE sds = $1`, [sds]);
        if (!result.rows.length)
            return undefined;
        return convertBigIntToNumber(this.mapRowToVenta(result.rows[0]));
    }
    async getBySPN({ spn }) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT * FROM venta WHERE sap = $1`, [spn]);
        return result.rows?.[0];
    }
    async getBySAP({ sap }) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT * FROM venta WHERE sap = $1`, [sap]);
        if (!result.rows.length)
            return undefined;
        return convertBigIntToNumber(this.mapRowToVenta(result.rows[0]));
    }
    async add({ input }) {
        ...
        const client = this.connection.getClient();
        const result = await client.queryObject(`INSERT INTO venta (sds, chip, stl, tipo_venta, sap, cliente_id, vendedor_id, multiple, plan_id, promocion_id, empresa_origen_id, fecha_creacion)
       ...
       RETURNING *`, [
        ...
        return convertBigIntToNumber(this.mapRowToVenta(newVenta));
    }
    async update({ id, input }) {
        ...
        values.push(id);
        const client = this.connection.getClient();
        const result = await client.queryObject(`UPDATE venta SET ${fields.join(", ")} WHERE venta_id = $${values.length}`, values);
    ...
    async delete({ id }) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`DELETE FROM venta WHERE venta_id = $1`, [id]);
        return result.rowCount !== undefined && result.rowCount > 0;
    }
    async getByVendedor({ vendedor }) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT * FROM venta WHERE vendedor_id = $1`, [vendedor]);
        return (result.rows || []).map((row) => this.mapRowToVenta(row));
    }
    async getByCliente({ cliente }) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT * FROM venta WHERE cliente_id = $1`, [cliente]);
        return (result.rows || []).map((row) => this.mapRowToVenta(row));
    }
    async getByPlan({ plan }) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT * FROM venta WHERE plan_id = $1`, [plan]);
        return (result.rows || []).map((row) => this.mapRowToVenta(row));
    }
    ...
    `;
        ...
        const mesResult = await client.queryObject(pais ? mesQuery.replace('$1', '$1') : mesQuery, pais ? [pais] : []);
        const ventasPorMes = (mesResult.rows || []).map((row) => ({
            mes: row.mes,
            cantidad: Number(row.cantidad),
        }));
        return {
    ...
    async getVentasUI(params) {
      ...
      `;
        ...
        return {
            ventas: uniqueRows.map((row) => convertBigIntToNumber(row)),
            total,
            page,
            limit,
        };
    ...
    `;
        ...
        const [estadosVenta, estadosCorreo, comentarios, portabilidad, lineaNueva, correo,] = await Promise.all([
            // Historial de estados de la venta
            client.queryObject(`SELECT * FROM estado WHERE venta_id = $1 ORDER BY fecha_creacion DESC`, [ventaId]),
            // Historial de estados del correo (si tiene SAP)
            client.queryObject(`SELECT * FROM estado_correo WHERE sap_id IS NOT DISTINCT FROM $1 ORDER BY fecha_creacion DESC`, [venta.sap]),
            // Comentarios
            client.queryObject(`SELECT c.*, p.nombre as autor_nombre, p.apellido as autor_apellido
         FROM comentario c
         INNER JOIN persona p ON c.usuarios_id = p.persona_id
         WHERE c.venta_id = $1
         ORDER BY c.fecha_creacion DESC`, [ventaId]),
            // Portabilidad
            client.queryObject(`SELECT * FROM portabilidad WHERE venta_id = $1`, [ventaId]),
            // Línea nueva
            client.queryObject(`SELECT * FROM linea_nueva WHERE venta_id = $1`, [ventaId]),
            // Datos del correo
            venta.sap
                ? client.queryObject(`SELECT * FROM correo WHERE sap_id = $1`, [venta.sap])
        ...
        return convertBigIntToNumber({
            ...
            linea_nueva: lineaNueva.rows && lineaNueva.rows.length > 0
                ...
                : null,
            historial_estados: (estadosVenta.rows || []).map((e) => ({
            ...
            historial_correo: (estadosCorreo.rows || []).map((e) => ({
...
//# sourceMappingURL=ventaPostgreSQL.js.map