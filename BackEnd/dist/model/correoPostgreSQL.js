...
export class CorreoPostgreSQL {
  ...
  `;
    ...
    async getById({ id }) {
        ...
        try {
            const client = this.connection.getClient();
            const result = await client.queryObject(this.baseSelect + " WHERE sap_id = $1", [id]);
    ...
    async getBySAP({ sap }) {
        ...
        try {
            const client = this.connection.getClient();
            const result = await client.queryObject(this.baseSelect + " WHERE sap_id = $1", [sap]);
            return result.rows.length > 0 ? result.rows[0] : undefined;
        }
        catch (error) {
    ...
    async add(params) {
        ...
        try {
            ...
            try {
                ...
                // Insertar correo
                await client.queryObject(`INSERT INTO correo (
            sap_id,
            sap,
            telefono_contacto,
            telefono_alternativo,
            ...
            comentario_cartero
                ...
                await client.queryObject(`INSERT INTO estado_correo (
            sap_id,
            estado,
            descripcion,
            fecha_creacion,
    ...
    async update(params) {
        ...
        try {
            ...
            await client.queryObject(`UPDATE correo SET ${fields.join(", ")} WHERE sap_id = $${paramIndex}`, values);
    ...
    async delete(params) {
        ...
        try {
            ...
            try {
                ...
                await client.queryObject(`DELETE FROM estado_correo WHERE sap_id = $1`, [id]);
                this.logSuccess("Estados del correo eliminados", { id });
                // 2. Eliminar el correo
                const correoResult = await client.queryObject(`DELETE FROM correo WHERE sap_id = $1 RETURNING sap_id`, [id]);
    ...
    async getByLocalidad({ localidad }) {
        ...
        try {
            ...
            const result = await client.queryObject(this.baseSelect +
                ` WHERE localidad ILIKE $1 ORDER BY fecha_creacion DESC`, [`%${localidad}%`]);
            return result.rows || [];
        }
        catch (error) {
    ...
    async getByDepartamento({ departamento }) {
        ...
        try {
            const client = this.connection.getClient();
            const result = await client.queryObject(this.baseSelect +
                ` WHERE departamento ILIKE $1 ORDER BY fecha_creacion DESC`, [`%${departamento}%`]);
            return result.rows || [];
        }
        catch (error) {
    ...
    async getProximosAVencer({ dias = 3 }) {
        ...
        try {
           ...
           ORDER BY fecha_limite ASC`, [dias]);
            return result.rows || [];
        }
        catch (error) {
     ...
     */
    async getByCodigoPostal({ codigoPostal }) {
        if (!codigoPostal) {
        ...
        try {
            const client = this.connection.getClient();
            const result = await client.queryObject(this.baseSelect +
                ` WHERE codigo_postal = $1 ORDER BY fecha_creacion DESC`, [codigoPostal]);
            return result.rows || [];
        }
        catch (error) {
    ...
    async count() {
        try {
            const client = this.connection.getClient();
            const result = await client.queryObject(`SELECT COUNT(*) as count FROM correo`);
    ...
    async countByEstado({ estado }) {
        ...
        try {
            const client = this.connection.getClient();
            const result = await client.queryObject(`SELECT COUNT(DISTINCT c.sap_id) as count
         FROM correo c
         INNER JOIN estado_correo ec ON c.sap_id = ec.sap_id
         WHERE ec.estado = $1`, [estado]);
            return result.rows.length > 0 ? Number(result.rows[0].count) : 0;
        }
        catch (error) {
...
//# sourceMappingURL=correoPostgreSQL.js.map