...
export class EstadoVentaPostgreSQL {
    connection;
    ...
    async getByVentaId({ venta_id }) {
        ...
        try {
         ...
         FROM estado
         WHERE venta_id = $1
         ORDER BY fecha_creacion DESC`, [venta_id]);
            return result.rows.map(this.mapRowToEstadoVenta) || [];
        }
        catch (error) {
    ...
    async getLastByVentaId({ venta_id }) {
        ...
        try {
            const client = this.connection.getClient();
            const result = await client.queryObject(`SELECT estado_id, venta_id, estado, descripcion, fecha_creacion, usuario_id
         FROM estado
         WHERE venta_id = $1
         ORDER BY fecha_creacion DESC
         LIMIT 1`, [venta_id]);
    ...
    async add({ input }) {
        ...
        try {
            ...
            // });
            const result = await client.queryObject(`INSERT INTO estado (venta_id, estado, descripcion, fecha_creacion, usuario_id)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING estado_id, venta_id, estado, descripcion, fecha_creacion, usuario_id`, [venta_id, estado, descripcion, new Date(), usuario_id]);
            // DEBUG: Descomentar para debugging
    ...
    async update({ id, input }) {
        ...
        try {
            ...
            values.push(id);
            const result = await client.queryObject(`UPDATE estado
         SET ${fields.join(", ")}
         WHERE estado_id = $${paramIndex}
         RETURNING estado_id`, values);
            if (!result.rows || result.rows.length === 0) {
    ...
    async delete({ id }) {
        ...
        try {
            ...
            const result = await client.queryObject(`DELETE FROM estado WHERE estado_id = $1 RETURNING estado_id`, [id]);
            if (!result.rows || result.rows.length === 0) {
     ...
     */
    async getLastStateForAllVentas() {
        try {
            const client = this.connection.getClient();
            const result = await client.queryObject(`SELECT DISTINCT ON (venta_id)
           estado_id,
           venta_id,
           estado,
           descripcion,
         ...
         ORDER BY venta_id, fecha_creacion DESC`);
            ...
            this.logError("Error al obtener últimos estados de todas las ventas", error);
            throw error;
        }
    }
    /**
    ...
    async getByFechaRango(params) {
        ...
        try {
         ...
         ORDER BY fecha_creacion DESC`, [fechaInicio, fechaFin]);
            return result.rows.map(this.mapRowToEstadoVenta) || [];
        }
        catch (error) {
    ...
    async getByEstado({ estado }) {
        ...
        if (!validation.success) {
            throw new Error(`Estado inválido: ${estado}. Estados válidos: ${EstadoVentaEnum.options.join(", ")}`);
        }
        try {
            const client = this.connection.getClient();
            const result = await client.queryObject(`SELECT
         ...
         FROM estado
         WHERE estado = $1
         ORDER BY fecha_creacion DESC`, [normalizedEstado]);
            return result.rows.map(this.mapRowToEstadoVenta) || [];
        }
        catch (error) {
    ...
    async getEstadisticasGenerales() {
        try {
            const client = this.connection.getClient();
            // Total de estados
            const totalResult = await client.queryObject(`SELECT COUNT(*) as count FROM estado`);
            // Estados por tipo
            const estadosPorTipoResult = await client.queryObject(`SELECT estado, COUNT(*) as cantidad
         FROM estado
         GROUP BY estado
         ORDER BY cantidad DESC`);
            // Estados por mes (últimos 12 meses)
            const estadosPorMesResult = await client.queryObject(`SELECT TO_CHAR(fecha_creacion, 'YYYY-MM') as mes, COUNT(*) as cantidad
         FROM estado
    ...
    async getByMultipleFilters(params) {
        const { venta_id, estado, fechaInicio, fechaFin, usuario_id, page = 1, limit = 10, } = params;
        if (page < 1 || limit < 1) {
            throw new Error("page y limit deben ser mayores a 0");
        }
        const conditions = [];
        ...
        try {
            ...
            try {
                ...
                estados.forEach((estado) => {
                    ...
                    values.push(venta_id, estadoValor.replace(/_/g, " "), descripcion || null, new Date(), usuario_id);
                });
                const result = await client.queryObject(`INSERT INTO estado (venta_id, estado, descripcion, fecha_creacion, usuario_id)
           VALUES ${placeholders.join(", ")}
           RETURNING estado_id, venta_id, estado, descripcion, fecha_creacion, usuario_id`, values);
                // ✅ COMMIT EXITOSO
     ...
     */
    async validateVentaIdsBulk(ventaIds) {
        if (ventaIds.length === 0)
            return;
        const client = this.connection.getClient();
        const placeholders = ventaIds.map((_, i) => `$${i + 1}`).join(", ");
        const result = await client.queryObject(`SELECT venta_id FROM venta WHERE venta_id IN (${placeholders})`, ventaIds);
        const foundIds = new Set(result.rows.map((row) => Number(row.venta_id)));
        const missingIds = ventaIds.filter((id) => !foundIds.has(Number(id)));
        if (missingIds.length > 0) {
            throw new Error(`Las siguientes ventas no existen: ${missingIds.join(", ")}`);
        }
    }
    /**
     * Valida múltiples usuario_ids en una sola query
     */
    async validateUsuarioIdsBulk(usuarioIds) {
        if (usuarioIds.length === 0)
            return;
        const client = this.connection.getClient();
        const placeholders = usuarioIds.map((_, i) => `$${i + 1}`).join(", ");
        const result = await client.queryObject(`SELECT persona_id FROM usuario WHERE persona_id IN (${placeholders})`, usuarioIds);
        const foundIds = new Set(result.rows.map((row) => row.persona_id));
     ...
     */
    async validateEstadosBulk(estados) {
        if (estados.length === 0)
            return;
        const client = this.connection.getClient();
        ...
        const result = await client.queryObject(`SELECT estado FROM tipo_estado WHERE estado IN (${placeholders})`, estadosNormalizados);
        const foundEstados = new Set(result.rows.map((row) => row.estado));
        const missingEstados = estadosNormalizados.filter((e) => !foundEstados.has(e));
        if (missingEstados.length > 0) {
            throw new Error(`Los siguientes estados no son válidos: ${missingEstados.join(", ")}`);
        }
    }
    // ======================
     ...
     */
    async validateVentaId(venta_id) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT venta_id FROM venta WHERE venta_id = $1`, [venta_id]);
        if (!result.rows || result.rows.length === 0) {
     ...
     */
    async validateUsuarioId(usuario_id) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT persona_id FROM usuario WHERE persona_id = $1 AND estado = 'ACTIVO'`, [usuario_id]);
        if (!result.rows || result.rows.length === 0) {
            throw new Error(`El usuario con ID ${usuario_id} no existe o no está activo`);
        }
    }
    /**
...
//# sourceMappingURL=estadoVentaPostgreSQL.js.map