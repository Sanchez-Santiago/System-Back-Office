...
export class UsuarioPostgreSQL {
  ...
  `;
    ...
    logError(message, error, throwInDev = false) {
        const isDev = process.env.MODO === "development";
        if (isDev) {
    ...
    async updatePassword(params) {
        ...
        const client = this.connection.getClient();
        const result = await client.queryObject(`UPDATE password
       SET password_hash = $2, fecha_creacion = $3
       WHERE usuario_persona_id = $1 AND activa = true
       RETURNING password_id`, [id, newPasswordHash, new Date()]);
        if (!result.rows || result.rows.length === 0) {
    ...
    async getPasswordHash({ id }) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT password_hash
       FROM password p
    ...
    // ======================
    async getPasswordHistory({ id, limit = 10 }) {
        const client = this.connection.getClient();
        const limitValue = limit || 10;
        const result = await client.queryObject(`SELECT password_hash, fecha_creacion
       FROM password
       WHERE usuario_persona_id = $1
       ORDER BY fecha_creacion DESC
       LIMIT $2`, [id, limitValue]);
    ...
    // ======================
    async isPasswordUsedBefore({ id, passwordHash }) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT 1
       FROM password
       WHERE usuario_persona_id = $1 AND password_hash = $2`, [id, passwordHash]);
        return result.rows.length > 0;
    }
    // ======================
    // GET FAILED ATTEMPTS
    // ======================
    async getFailedAttemptsDB({ id }) {
        const client = this.connection.getClient();
        const result = await client.queryObject(`SELECT p.intentos_fallidos
      FROM password p
      INNER JOIN usuario u ON p.usuario_persona_id = u.persona_id
      ...
      LIMIT 1;`, [id]);
        ...
        const row = result.rows[0];
        return row.intentos_fallidos || 0;
    }
    // ======================
    // INCREMENT FAILED ATTEMPTS
    // ======================
    async incrementFailedAttemptsDB({ id }) {
        const currentAttempts = await this.getFailedAttemptsDB({ id });
        const newAttempts = currentAttempts + 1;
        const client = this.connection.getClient();
        await client.queryObject(`UPDATE password
             SET intentos_fallidos = $1
             WHERE usuario_persona_id = $2`, [newAttempts, id]);
    }
    // ======================
    // RESET FAILED ATTEMPTS
    // ======================
    async resetFailedAttemptsDB({ id }) {
        const client = this.connection.getClient();
        await client.queryObject(`UPDATE password
             SET intentos_fallidos = 0
             WHERE usuario_persona_id = $1`, [id]);
    }
    // ======================
    // ADD METHOD
    // ======================
    async add({ input }) {
        ...
        try {
            ...
            tipo_documento, nacionalidad, genero
                ...
                await client.queryObject(`INSERT INTO usuario (
            ...
            celula, estado
                ...
                await client.queryObject(`INSERT INTO password (
            password_hash, usuario_persona_id, fecha_creacion, activa
          ) VALUES ($1, $2, $3, $4)`, [input.password_hash, personaId, now, true]);
                ...
                if (input.permisos && input.permisos.length > 0) {
                    ...
                    for (const permisoId of permisosIds) {
                        await client.queryObject(`INSERT INTO permisos_has_usuario (permisos_id, persona_id)
               VALUES ($1, $2)`, [permisoId, personaId]);
                    }
                ...
                if (input.rol === "VENDEDOR") {
                    await client.queryObject(`INSERT INTO vendedor (usuario_id) VALUES ($1)`, [personaId]);
                }
                else if (input.rol === "SUPERVISOR") {
                    await client.queryObject(`INSERT INTO supervisor (usuario_id) VALUES ($1)`, [personaId]);
                }
                else if (input.rol === "BACK_OFFICE") {
                    await client.queryObject(`INSERT INTO back_office (usuario) VALUES ($1)`, [personaId]);
                }
    ...
    // CONSULTAR PERMISOS
    // ======================
    async consultarPermisos(permisos) {
        const client = this.connection.getClient();
        const placeholders = permisos.map((_, index) => `$${index + 1}`).join(",");
        const result = await client.queryObject(`SELECT permisos_id
       FROM permisos
       WHERE nombre IN (${placeholders})`, permisos);
        if (!result.rows || result.rows.length === 0) {
            this.logWarning("No se encontraron permisos en la BD", { permisos });
            return [];
        }
        return result.rows.map((row) => row.permisos_id);
    }
    // ======================
    // UPDATE METHOD
    // ======================
    ...
    async delete(params) {
        ...
        try {
            ...
            try {
                ...
                await client.queryObject(`DELETE FROM permisos_has_usuario WHERE persona_id = $1`, [id]);
                // 2. Eliminar historial de contraseñas
                await client.queryObject(`DELETE FROM password WHERE usuario_persona_id = $1`, [id]);
                // 3. Eliminar el registro de usuario
                const usuarioResult = await client.queryObject(`DELETE FROM usuario WHERE persona_id = $1 RETURNING persona_id`, [id]);
...
//# sourceMappingURL=usuarioPostgreSQL.js.map