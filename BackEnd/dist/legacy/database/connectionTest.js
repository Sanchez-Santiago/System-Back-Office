...
import { testTcpConnection, retryWithBackoff, formatConnectionTestResult, identifyErrorType } from "../../Utils/connectionTester.ts";
...
export class MySQLConnectionTester {
    ...
    async testFullConnection(options = {}) {
        ...
        const { retries = 3, timeout = 10000, verbose = false, retryDelay = 2000 } = options;
        ...
        try {
            ...
            const authResult = await retryWithBackoff(() => this.testAuthentication({ timeout, verbose }), { retries, retryDelay, verbose });
     ...
     */
    async testNetworkConnection(options) {
     ...
     */
    async testAuthentication(options) {
        const startTime = Date.now();
     ...
     */
    async testDatabaseAccess(options) {
        const startTime = Date.now();
        const { timeout = 5000, verbose = false } = options;
        try {
            if (verbose) {
     ...
     */
    async checkCriticalTables(criticalTables) {
        const startTime = Date.now();
        try {
            const client = new Client();
            await client.connect({
 ...
 */
export function createMySQLTesterFromEnv() {
