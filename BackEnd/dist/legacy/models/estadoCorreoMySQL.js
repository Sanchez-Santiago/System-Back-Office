 ...
 */
export class EstadoCorreoMySQL {
    connection;
    ...
    async getAll(params) {
      ...
      `;
            ...
            logger.debug("getAll estados correo (último por SAP) - Query:", query);
    ...
    async update(params) {
        try {
            ...
            const result = await this.connection.execute(`UPDATE estado_correo SET ${fields.join(", ")} WHERE estado_correo_id = ?`, values);
            logger.info(`Estado correo actualizado - Affected rows: ${result.affectedRows || 0}`);
    ...
    async delete(params) {
        try {
            ...
            // Eliminar estado
            const result = await this.connection.execute(`DELETE FROM estado_correo WHERE estado_correo_id = ?`, [id]);
    ...
    async getByFechaRango(params) {
        try {
            logger.debug(`getByFechaRango: ${params.fechaInicio} - ${params.fechaFin}`);
            const result = await this.connection.execute(`
        SELECT
          estado_correo_id,
          sap_id,
    ...
    async actualizarUbicacion(params) {
        try {
            logger.info(`Actualizando ubicación: ${params.id} -> ${params.ubicacion}`);
            return await this.update({
                id: params.id,
                input: {
...
//# sourceMappingURL=estadoCorreoMySQL.js.map