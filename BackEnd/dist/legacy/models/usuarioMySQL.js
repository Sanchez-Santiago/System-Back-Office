...
export class UsuarioMySQL {
  ...
  `;
    ...
    async consultarPermisos(permisos) {
      ...
      `, permisos);
        return (result.rows ?? []).map((r) => r.permisos_id);
    }
    // ======================================================
        ...
        `, [input.password_hash, personaId, now]);
          ...
          `, [permisoId, personaId]);
            ...
            if (input.rol === "VENDEDOR") {
                await this.connection.execute(`INSERT INTO vendedor (usuario_id) VALUES (?)`, [personaId]);
            }
            else if (input.rol === "SUPERVISOR") {
                await this.connection.execute(`INSERT INTO supervisor (usuario_id) VALUES (?)`, [personaId]);
            }
            else if (input.rol === "BACK_OFFICE") {
                await this.connection.execute(`INSERT INTO back_office (usuario) VALUES (?)`, [personaId]);
            }
            await this.connection.execute("COMMIT");
            const usuario = await this.getById({ id: personaId });
            if (!usuario)
                throw new Error("Usuario no recuperado");
            return usuario;
        }
        catch (error) {
    ...
    async update({ id, input, }) {
        ...
        try {
            ...
            if (input.permisos) {
                await this.connection.execute(`DELETE FROM permisos_has_usuario WHERE persona_id = ?`, [id]);
                const permisosIds = await this.consultarPermisos(input.permisos);
                for (const permisoId of permisosIds) {
                    await this.connection.execute(`
            INSERT INTO permisos_has_usuario (permisos_id, persona_id)
    ...
    // ======================================================
    async delete({ id }) {
        const result = await this.connection.execute(`DELETE FROM persona WHERE persona_id = ?`, [id]);
    ...
    // ======================================================
    async getPasswordHistory({ id, limit = 5, }) {
        const result = await this.connection.execute(`
...
//# sourceMappingURL=usuarioMySQL.js.map