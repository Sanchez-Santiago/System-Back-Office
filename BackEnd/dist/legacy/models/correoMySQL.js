...
export class CorreoMySQL {
    ...
    async update(params) {
        try {
            ...
            const result = await this.connection.execute(`UPDATE correo SET ${fields.join(", ")} WHERE sap_id = ?`, values);
            logger.info(`Correo actualizado - Affected rows: ${result.affectedRows || 0}`);
    ...
    async delete(params) {
        try {
            ...
            // Eliminar correo
            const result = await this.connection.execute(`DELETE FROM correo WHERE sap_id = ?`, [id]);
...
//# sourceMappingURL=correoMySQL.js.map