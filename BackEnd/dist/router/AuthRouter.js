...
export function authRouter(userModel) {
    ...
    router.post("/usuario/login", async (req, res) => {
        try {
            ...
            if (!input || !input.user) {
                throw new Error("Estructura de datos inválida. Se espera { user: {...} }");
            }
    ...
    router.post("/usuario/register", authMiddleware(userModel), rolMiddleware("SUPERADMIN"), async (req, res) => {
        try {
            const input = req.body;
            if (!input || !input.user) {
                throw new Error("Datos de usuario inválidos. Se espera { user: {...} }");
            }
    ...
    router.get("/usuario/verify", async (req, res) => {
        try {
            ...
            const user = await userModel.getById({ id: payload.id });
            if (!user) {
                res.status(401).json({
                    success: false,
    ...
    router.post("/usuario/refresh", async (req, res) => {
        try {
            let token = req.cookies?.token;
            if (!token) {
                const authHeader = req.headers.authorization;
    ...
    router.patch("/usuarios/:id/password", authMiddleware(userModel), async (req, res) => {
        try {
            const { id } = req.params;
            if (!id || id.trim() === "") {
                res.status(400).json({
