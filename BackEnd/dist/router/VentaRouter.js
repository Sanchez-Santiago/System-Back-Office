...
export function ventaRouter(ventaModel, userModel, correoModel, lineaNuevaModel, portabilidadModel, clienteModel, planModel, promocionModel, estadoVentaModel, pgClient) {
    const router = express.Router();
    const ventaController = new VentaController(ventaModel, clienteModel, correoModel, lineaNuevaModel, portabilidadModel, planModel, promocionModel, estadoVentaModel);
    ...
    router.get("/ventas", authMiddleware(userModel), rolMiddleware(...ROLES_MANAGEMENT), async (req, res) => {
        try {
    ...
    router.get("/ventas/estadisticas", authMiddleware(userModel), rolMiddleware(...ROLES_MANAGEMENT), async (req, res) => {
        try {
            const paisParam = req.query.pais;
            const user = req.user;
            const rol = user?.rol?.toUpperCase();
    ...
    router.get("/ventas/fechas", authMiddleware(userModel), async (req, res) => {
        try {
    ...
    router.get("/ventas/sds/:sds", authMiddleware(userModel), async (req, res) => {
        try {
    ...
    router.get("/ventas/sap/:sap", authMiddleware(userModel), async (req, res) => {
        try {
    ...
    router.get("/ventas/vendedor/:vendedor", authMiddleware(userModel), async (req, res) => {
        try {
    ...
    router.get("/ventas/cliente/:cliente", authMiddleware(userModel), async (req, res) => {
        try {
    ...
    router.get("/ventas/plan/:plan", authMiddleware(userModel), async (req, res) => {
        try {
    ...
    router.get("/ventas/:id/detalle", authMiddleware(userModel), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    router.get("/ventas/ui", authMiddleware(userModel), rolMiddleware(...ROLES_MANAGEMENT), async (req, res) => {
        try {
    ...
    router.get("/ventas/:id", authMiddleware(userModel), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    router.post("/ventas", authMiddleware(userModel), async (req, res) => {
        try {
            const body = req.body;
            const user = req.user;
            const result = await ventaController.createFullVenta(body, user.id);
    ...
    router.put("/ventas/:id", authMiddleware(userModel), rolMiddleware(...ROLES_ADMIN), async (req, res) => {
        try {
            const { id } = req.params;
            logger.debug(`PUT /ventas/${id}`);
            const result = VentaUpdateSchema.safeParse(req.body);
            if (!result.success) {
                logger.error("PUT /ventas/:id validation error:", result.error.errors);
                res.status(400).json({
                    success: false,
    ...
    router.delete("/ventas/:id", authMiddleware(userModel), rolMiddleware(...ROLES_ADMIN), async (req, res) => {
        try {
            const { id } = req.params;
...
//# sourceMappingURL=VentaRouter.js.map