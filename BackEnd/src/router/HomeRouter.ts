...
<body class="antialiased p-4 md:p-8">
    <div class="max-w-7xl mx-auto space-y-8">
        <header class="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div class="bento-card p-6 flex flex-col items-center justify-center text-center">
                ...
                <p class="text-slate-500 text-xs uppercase tracking-widest font-bold mt-1">Node Cluster</p>
            </div>
        </header>
    ...
    <script>
                ...
                if (ep.request) {
                    ...
                    if (fields) {
                        spec.innerHTML = Object.entries(fields).map(([k, v]) => 
                            '<div class="flex justify-between border-b border-slate-800/80 last:border-0 py-1.5 px-1 hover:bg-white/5 transition-colors"><span class="flex flex-col"><span class="text-indigo-200 font-semibold">' + k + (v.required ? '<span class="text-rose-500">*</span>' : '') + '</span><span class="text-[9px] text-slate-500">' + (v.desc || '') + '</span></span><span class="text-[9px] text-blue-400/80 italic font-mono self-center uppercase">' + v.type + '</span></div>'
                        ).join('');
                    } else { spec.classList.add('hidden'); }
                    const example = ep.request.example;
                    if (example) { reqSec.querySelector('pre').innerText = JSON.stringify(example, null, 2); }
                    else { reqSec.querySelector('.example-area').classList.add('hidden'); }
                }
                if (ep.response) {
                    const resSec = clone.querySelector('.response-section');
                    resSec.classList.remove('hidden');
                    const example = ep.response.example;
                    if (example) { resSec.querySelector('pre').innerText = JSON.stringify(example, null, 2); }
                    else { resSec.classList.add('hidden'); }
                }
        ...
        function copyContent(btn) {
            const pre = btn.closest('.example_area').querySelector('pre');
...
routerHome.get('/api-docs-json', async (req: Request, res: Response) => {
  try {
    const jsonPath = path.join(process.cwd(), 'DOCUMENTATION.json');
    const data = fs.readFileSync(jsonPath, 'utf-8');
    res.json(JSON.parse(data));
  } catch (_error) {
    res.status(404).json({ error: 'Documentation JSON not found.' });
  }
});

routerHome.get('/home', (req: Request, res: Response) => {
  res.redirect('/');
});
