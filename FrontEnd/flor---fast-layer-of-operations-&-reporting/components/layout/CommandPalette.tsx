...
import { useCommandPaletteViewModel } from '../../viewmodels/layout/useCommandPaletteViewModel';
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ onClose, onNavigate, onSearch, onAction }) => {
  const { state, actions } = useCommandPaletteViewModel({ onClose, onNavigate, onSearch, onAction });
      ...
      <div className="w-full max-w-[650px] bg-white/80 dark:bg-slate-900/90 backdrop-blur-2xl rounded-[3.5vh] shadow-[0_30px_100px_-20px_rgba(0,0,0,0.5)] border border-white/40 dark:border-white/10 overflow-hidden animate-in zoom-in-95 slide-in-from-top-4 duration-300 relative z-10">
        <div className="p-[2.5vh] border-b border-slate-100 dark:border-white/5 flex items-center gap-[2vh]">
          ...
          <input
            ...
            type="text"
            value={state.query}
            onChange={(e) => actions.handleQueryChange(e.target.value)}
        ...
        <div className="max-h-[50vh] overflow-y-auto p-[1.5vh] space-y-[0.5vh] no-scrollbar">
          {state.filteredCommands.length > 0 ? (
            state.filteredCommands.map((cmd, i) => (
              <button
                key={cmd.id}
                onMouseEnter={() => actions.setSelectedIndex(i)}
                onClick={() => { cmd.action(); onClose(); }}
                className={`w-full flex items-center justify-between p-[1.8vh] rounded-[2vh] transition-all ${i === state.selectedIndex ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'hover:bg-slate-50 dark:hover:bg-white/5 text-slate-600 dark:text-slate-400'}`}
              >
                <div className="flex items-center gap-[2vh]">
                  <span className="text-[2.5vh]">{cmd.icon}</span>
                  <div className="text-left">
                    <p className={`font-black uppercase tracking-tight ${i === state.selectedIndex ? 'text-white' : 'text-slate-800 dark:text-slate-200'}`}>{cmd.label}</p>
                    <p className={`text-[1.1vh] font-bold uppercase tracking-widest opacity-60 ${i === state.selectedIndex ? 'text-indigo-100' : ''}`}>{cmd.category}</p>
                  </div>
                </div>
            ...
            <div className="p-[4vh] text-center space-y-[2vh]">
              ...
              <p className="font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest leading-relaxed">
                ...
                <span className="text-slate-800 dark:text-slate-200">"{state.query}"</span> en toda la base.
              </p>
            </div>
          ) : (
