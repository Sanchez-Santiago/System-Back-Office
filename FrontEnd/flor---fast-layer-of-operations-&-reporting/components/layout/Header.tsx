...
function getPaisCodigo(pais: string | null | undefined): { codigo: string; color: string } | null {
  ...
  if (paisLower.includes('argentina')) {
    return { codigo: 'AR', color: 'bg-blue-500' };
  }
  if (paisLower.includes('uruguay')) {
    return { codigo: 'UY', color: 'bg-green-500' };
  }
  if (paisLower.includes('paraguay')) {
    return { codigo: 'PY', color: 'bg-red-500' };
  }
...
export const Header: React.FC<HeaderProps> = React.memo(({
  ...
  const { state, actions } = useHeaderViewModel();
  const { selectedCountry, setSelectedCountry, effectiveCountry, userCountry, isAdminView, countryOptions } = useCountry();
  ...
  return (
    <>
      {(state.showNotifications || state.showProfileMenu) && (
        <div 
          className="fixed inset-0 z-40 bg-slate-900/10 dark:bg-black/60 backdrop-blur-sm transition-all duration-500 animate-in fade-in"
          onClick={actions.closeMenus}
        ></div>
      )}
      ...
      <header className="sticky top-0 z-50 w-full px-[1vw] py-[1vh]">
        <div className="w-[98vw] mx-auto flex items-center justify-between glass-panel rounded-[3.2vh] px-[2vw] py-[1vh] relative min-h-[8.5vh]">
          ...
          <div className="flex items-center gap-[2vw] relative z-10">
            ...
            >
              ...
              <div className="flex flex-col justify-center">
                ...
                <div className="flex items-center gap-[0.5vw] mt-[0.2vh]">
                  ...
                  <span className="font-black text-slate-600 dark:text-slate-400 tracking-[0.4em] uppercase opacity-90 dark:opacity-80 text-[clamp(0.55rem,1.1vh,1.3rem)]">
                    v{APP_VERSION}
                  </span>
                  {isAdminView || !userCountry || permisoPrincipal === 'ADMIN' || permisoPrincipal === 'SUPERADMIN' ? (
                    <div className="relative ml-2">
                      <button
                        onClick={() => actions.setShowCountryDropdown(!state.showCountryDropdown)}
                      ...
                      {state.showCountryDropdown && (
                        <div className="absolute top-full left-0 mt-1 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 py-1 z-50 min-w-[100px]">
                          <button
                            onClick={() => { setSelectedCountry(null); actions.setShowCountryDropdown(false); }}
                            className={`w-full text-left px-3 py-1.5 text-xs font-bold hover:bg-slate-100 dark:hover:bg-slate-700 ${!selectedCountry ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-600' : ''}`}
                          >
                            ALL
                          </button>
                          {countryOptions.map((pais) => {
                            ...
                            return (
                              <button
                                key={pais}
                                onClick={() => { setSelectedCountry(pais); actions.setShowCountryDropdown(false); }}
                    ...
                    (() => {
                      ...
                      return paisInfo ? (
                        ...
                        <span className="ml-2 w-5 h-5 flex items-center justify-center rounded-full bg-slate-400 text-white text-xs" title="Sin país asignado">
                          🌐
                        </span>
                      );
                    })()
            ...
            <nav className="hidden lg:flex items-center bg-slate-100/50 dark:bg-white/5 backdrop-blur-md p-[0.6vh] rounded-[2vh] border border-slate-200 dark:border-white/10">
                ...
                <button
                  ...
                  className={`px-[1.8vw] py-[1.2vh] rounded-[1.5vh] font-black transition-all uppercase tracking-widest text-[clamp(0.7rem,1.4vh,1.5rem)] ${
                    activeTab === tab 
                      ...
                      : 'text-slate-700 dark:text-indigo-300/60 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-white/60 dark:hover:bg-white/5'
                  }`}
                >
                  {tab}
                </button>
          ...
          <div className="flex items-center gap-[1.5vw] relative z-10">
            <div className="relative">
              <button 
                ...
                className={`relative w-[5.5vh] h-[5.5vh] flex items-center justify-center rounded-[2.2vh] transition-all border shadow-sm ${state.showNotifications ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-slate-50 dark:bg-slate-800/40 text-slate-700 dark:text-indigo-400 border-slate-200 dark:border-white/10 hover:bg-indigo-50 dark:hover:bg-indigo-900/30'}`}
              >
                <svg className="w-[3vh] h-[3vh]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                {state.unreadCount > 0 && (
                  <span className="absolute -top-[0.2vh] -right-[0.2vh] min-w-[2.2vh] h-[2.2vh] flex items-center justify-center bg-rose-500 border-2 border-white dark:border-slate-900 rounded-full text-white font-black text-[clamp(0.55rem,0.9vh,0.8rem)] px-[0.4vh] leading-none shadow-lg">
                    {state.unreadCount > 99 ? '99+' : state.unreadCount}
                  </span>
                )}
              </button>
              {state.showNotifications && <NotificationCenter onClose={actions.closeMenus} onUnreadCountChange={actions.handleUnreadCountChange} />}
            </div>
            ...
            <div className="relative">
              ...
              >
                <div className="text-right hidden sm:block">
                  <p className={`font-black leading-none transition-colors text-[clamp(0.8rem,1.6vh,2.2rem)] ${state.showProfileMenu ? 'text-indigo-600' : 'text-slate-900 dark:text-white'}`}>
                  ...
                  >
                    ...
                    {state.showPermissionsTooltip && otrosPermisos && otrosPermisos.length > 0 && (
                ...
                <div className={`w-[6vh] h-[6vh] rounded-[1.8vh] border-2 overflow-hidden shadow-lg transition-all ${state.showProfileMenu ? 'border-indigo-600 scale-110 ring-4 ring-indigo-50 dark:ring-indigo-900/40' : 'border-slate-100 dark:border-white/20 group-hover:scale-105'}`}>
                  <img src="https://picsum.photos/100/100?random=1" alt="Avatar" className="w-full h-full object-cover" />
                </div>
              </div>
              {state.showProfileMenu && (
                <ProfileMenu
                  onClose={actions.closeMenus}
